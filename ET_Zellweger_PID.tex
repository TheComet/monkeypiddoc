...
%
%\begin{document}
\subsubsection{Vorgehen PID-Regler mit Matlab}\label{PIDExample}
...
$T3=10.4901s$\\
\\
Nun wird nach dem -135\textdegree ~Punkt auf dem Phasengang gesucht. Dies ist in Abbildung \ref{fig:PIDStep1} ersichtlich und ergibt uns folgende Frequenz: $0.2987s^{-1}$. 
...
\newpage
Bevor nun jedoch der offene Frequenzgang berechnet wird, muss $\beta$ bestimmt werden. Die genaue Berechnung dazu ist im Anhang, Kapitel \ref{ZellwegerBeta} auf Seite \pageref{ZellwegerBeta} ersichtlich. Nach Ausführen dieser Berechnung erhält man $\beta=0.3192$.

Mit $\beta$ ist es einem möglich, $Tnk=10.4902s$ und $Tvk=1.0688s$ zu berechnen.
...
%\newpage
Als nächster Schritt wird wieder die Tabelle \ref{table:TabÜberschwingen} auf Seite \pageref{table:TabÜberschwingen} zu Hilfe genommen. Daraus wird das gewünschte Überschwingen ausgewählt und die entsprechende Frequenz auf dem offenen Phasengang gesucht (siehe Abbildung \ref{fig:PIDStep3}). Die gefundene Frequenz lautet: $0.1317s^{-1}$.
...
\newpage
Zu der gefunden Frequenz liest man nun die Verstärkung von $0.3312$ heraus. Der Kehrwert davon ergibt den letzten Faktor $Krk=3.0194$. (Abbildung \ref{fig:PIDStep4})
...
Den genauen Verlauf für die Umrechnung zur Regelkonfomren Darstellung ist im Anhang, Sektor \ref{Bodekonf}, Seite \pageref{Bodekonf} zu finden.

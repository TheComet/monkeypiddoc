...
Phasengang kann aus der Übertragungsfunktion $H(s)$\eqref{Übertragungsfunktion} berechnet werden. 
Folgend werden die dazu verwendeten Formeln aufgeführt:

\[ %\label{Amplitudengang}
Amplitudengang~Strecke~A_{s}(\omega)= Ks\cdot\prod\limits_{m=1}^Z\frac{1}{\sqrt{1+{(\omega\cdot T_{m})}^{2}}}
\]

\[ %\label{Phasengang}
Phasengang~Strecke~\phi_{s}(\omega)=-1\cdot\sum\limits_{m=1}^Z\arctan \left( \omega\cdot T_{m}\right)
\]
...
$\phi_{x}$ ist abhängig davon, welcher Regler-Typ 
...
und Strecke $(Gs)$ in Serie, aber ohne Rückkoppelung. 
\[
Übertragungsfunktion\, PI-Regler\, \left( reglerkonform \right)Gr=Kr\cdot 
\left[ 1+\frac{1}{s\cdot Tn} \right]
\]

\[
Übertragungsfunktion\, PI-Regler\, (bodekonform)\, Gr=Kr\cdot 
\frac{1+s\cdot Tn}{s\cdot Tn}
\]

\[
Phasengang\, PI-Regler\, \phi_{r}=\arctan \left( \omega\cdot Tn 
\right)-\frac{\pi}{2}
\]

\[
Amplitudengang\, PI-Strecke\, A_{r}=\, Kr\frac{\sqrt {1+{(\omega\cdot 
Tn)}^{2}} }{\omega\cdot Tn}
\]


\[
Phasengang\, Regelkreis\, offen\, \, \, \phi_{o}=\, 
\phi_{s}+\phi_{r}\, 
\]

\[
Amplitudentang\, Regelkreis\, offen\, \, \, A_{ 
o}=A_{s}\cdot A_{r}
\]
...
gewählten Überschwingens und des dazu gehörigen Winkels.
...
\centering
	...
	\hline
	Dämpfmass d & $\phi_{rand}$ & $\phi_{st}$ & Überschwingen \\
	\hline
...
Bei 0\% Überschwingen muss beispielsweise der Winkel $\phi_{st}$ 
gleich -103.7\textdegree ~auf dem Phasengang gesucht werden. Bei der 
gefundenen Frequenz wird der dazu gehörige Wert aus dem Amplitudengang 
...
PID-Regler folglich die Formel der Übertragungsfunktion \eqref{Übertragungsfunktion} des PID-Reglers 
...
1 Dekade höher als $1/Tvk$ gewählt werden. Beim PID-Regler wird die 
Bodekonforme Darstellung verwendet, weil $Tnk$ und $Tvk$ vorhanden sind, $Tn$ und $Tk$ jedoch noch nicht. Eine genauere Erläueterung dieser ist im Anhang Seite \pageref{Bodekonf} Kapitel \ref{Bodekonf} zu finden. Für die Berechnung des Amplitudenganges wird $Krk=1$ verwendet: 
...
\[
Übertragungsfunktion\, PID-Regler\, (Reglerkonform)\, Gr=Kr\cdot \left[ 
1+\frac{1}{s\cdot Tn}+\frac{s\cdot Tv}{1+s\cdot Tp} \right]
\]

\[
Übertragungsfunktion\, PID-Regler\, (bodekonform)\, Gr=Krk\cdot \left[ 
\frac{\left( 1+s\cdot Tnk \right)\cdot (1+s\cdot Tvk)}{s\cdot Tnk\cdot (1+s\cdot 
Tp)} \right]
\]

\[
Phasengang\, PID-Regler\, \phi_{r}=\arctan {\left( \omega\cdot Tnk 
\right)+arctan\left(\omega\cdot Tvk \right)-\mathrm{arctan}(\omega\cdot 
Tp)}-\frac{\pi}{2}
\]

\[
Amplitudengang\, PID-Regler\, A_{r}=\, Krk\frac{\sqrt {1+\left( 

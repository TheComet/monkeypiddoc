...
\text{Amplitudengang Strecke}~A_{S}(\omega)= Ks\cdot\prod\limits_{m=1}^N\frac{1}{\sqrt{1+{(\omega\cdot T_{m})}^{2}}}
\]

\[ %\label{Phasengang}
\text{Phasengang Strecke}~\phi_{S}(\omega)=-1\cdot\sum\limits_{m=1}^N\arctan \left( \omega\cdot T_{m}\right)
\]

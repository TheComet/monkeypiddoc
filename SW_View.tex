...
\end{figure}
\newpage
\textbf{OutputPanel:} Dieses Panel wurde im GridBagLayout erstellt und besitzt einen Rahmen mit dem Titel "`Ausgabe"'. In einer übersichtlichen Tabelle werden die Kennwerte jeder simulierten Kurve aufgelistet. Diese Tabelle besteht aus den sechs Spalten mit den folgenden Inhalten:
\begin{enumerate}
...
Im folgenden Ausschnitt aus dem Klassendiagramm ist der Zusammenhang der Klasse \textit{View} zu den Panels und den ganzen Swing-Komponenten dargestellt. Für das gesamte Klassendiagramm wird auf den Anhang \ref{klassendiagramm} verwiesen.

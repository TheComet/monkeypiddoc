...
\subsection{View}
Die Klasse \empth{View} und ihre Unterklassen sind zuständig für die Darstellung der Benutzeroberfläche. Am oberen Fensterrand befindet sich die \empth{MenuBar}, welche sich über die gesamte Breite erstreckt. Sie enthält Einstellungsmöglichkeiten und weiterführende Informationen. Auf der linken Seite befinden sich zwei Panels für die Ein- und Ausgabe. Im \empth{InputPanel} können die Eingabeparameter eingetragen, der gewünschte Regler und das Überschwingen gewählt und die Simulation mit einem Button gestartet werden. Das \empth{OutputPanel} stellt alle berechneten Graphen mit der Angabe ihrer Kennwerte in einer Tabelle dar und besitzt einen Trimmer für das manuelle justieren des Phasenwinkels. Auf der rechten Seite des Fensters werden die Graphen mit der Klasse \empth{GraphPanel} angezeigt. Unter diesem Plot befindet sich das \empth{GraphDisplayPanel} und das \empth{GraphSettingPanel}, welche beide für für die Auswahl der darzustellenden Kurven verantwortlich sind. Im folgenden Abschnitt wird die Darstellung des GUIs (Graphical User Interface) genauer erläutert.
...
\begin{enumerate}
	\item \empth{GridBagLayout}: Er ist der mächtigste LayoutMangager aus dem Java AWT-Package und ermöglicht es, die Komponenten in ein anpassbares Raster (Grid) zu setzen. Die einzelnen Spalten können dabei unterschiedlich breit und die einzelnen Zeilen unterschiedlich hoch sein. Die Komponenten werden in eine Zelle gesetzt und können von dort in X- und Y-Richtung weitere Zellen überlappen. Weiter gibt es viele zusätzliche Anpassungsmöglichkeiten.
	\item \empth{GridLayout}: Setzt die Komponenten in ein gleichmässiges Raster (Grid), bei dem alle Zellen die selbe Grösse haben.
	\item \empth{BorderLayout}: Besitzt fünf Bereiche (Norden, Süden, Osten, Westen, Zentrum), in welche Komponenten platziert werden können.
	\item \empth{WrapLayout}: Alle Komponenten werden nacheinander von links nach rechts eingefüllt. Beim Verändern der Fenstergrösse gibt es Zeilenumbrüche, falls die Breite des Panels nicht mehr ausreicht. Dieses Layout gehört nicht zu den Standard-Typen und wird bei der Erläuterung des \empth{GraphDisplayPanels} genauer beschrieben.
\end{enumerate}

Für weitere Informationen zu Layoutmanagern wird auf die Dokumentation von Oracle verwiesen \cite{OracleDoc}.

\textbf{Aufbau der Benutzeroberfläche}\\
Der Aufbau des GUIs wurde so erstellt, dass die gesamte grafische Benutzeroberfläche in einem JFrame platziert ist. Dieses Frame enthält jeweils eine Instanz der  Klasse \empth{MenuBar} und \empth{View}. Die \empth{View} besteht wiederum aus den vier JPanels \empth{InputOutputPanel}, \empth{GraphPanel}, \empth{GraphDisplayPanel} und \empth{GraphSettingPanel}. Das \empth{InputOutputPanel} fasst das \empth{InputPanel} und das \empth{OutputPanel} zu einem gemeinsamen JPanel zusammen. Auf die Layoutmanager wird bei der Beschreibung der einzelnen Panels genauer eingegangen. \\
...
Die Benutzeroberfläche beinhaltet, wie vorhin beschrieben, mehrere Panels. Das \empth{Input\-Panel} dient zur Eingabe der Kenngrössen der Regelstrecke und das \empth{OutputPanel} zur Anzeige der berechneten Reglerparameter und zur nachträglichen Manipulation des Phasenwinkels. Das \empth{GraphPanel} visualisiert die Schrittantwort, das \empth{GraphDisplayPanel} ermöglicht die Auswahl der anzuzeigenden Kurven und das \empth{GraphSettingPanel} besitzt drei Buttons für die Einstellungen zum Graphen. Ein Grossteil der verwendeten Komponenten stammt aus der Swing Klasse von Java, alle zusätzlichen werden speziell erläutert. Folgend werden die Panels genauer beschrieben:\\
...
\textbf{InputOutputPanel:} Die Aufgabe dieses im GridBagLayout erstellten Panels besteht einzig darin, das \empth{InputPanel} und das \empth{OutputPanel} in einem gemeinsamen Panel zu vereinen. Dieses Panel wurde erstellt, um die Handhabung im GridBagLayout der Klasse \empth{View} zu verbessern. \\

\textbf{InputPanel:} Dieses Panel wurde im GridBagLayout erstellt und besitzt einen Rahmen mit dem Titel "`Eingabe"'. Es enthält zuoberst die drei Eingabefelder für die Parameter \empth{Tu} (1), \empth{Tg} (2) und \empth{Ks} (3) der Regelstrecke. Anschliessend folgt eine leere Zeile, die zu Beginn keinen Inhalt hat. Sie zeigt nach einer Simulation den Grad der Berechnung an und bei fehlerhaften Eingaben rote Hinweise für den Anwender. Als nächstes folgt das Dropdown-Menü für die Auswahl des Reglers (4) (\empth{I}, \empth{PI} oder \empth{PID}). In den nächsten beiden Textfeldern (4 und 5) kann das Überschwingen und die parasitäre Zeitkonstante eingetragen werden. Diese beiden Eingaben  sind abhängig von der Auswahl des Reglers und deshalb nicht für alle Regler aktiviert. Wenn die Felder deaktiviert sind, werden sie ausgegraut und es können keine Werte eingetragen werden. Beim I-Regler werden beide und beim PI-Regler nur das Feld für die parasitäre Zeitkonstante deaktiviert. Beim PID-Regler sind beide aktiviert. Am Ende dieses Panels befindet sich über die gesamte Breite die Schaltfläche zum Starten der Simulation (7).
...
\begin{enumerate}
	\item \empth{Name}: Als erstes steht ein grosser farbiger Kreis in der Farbe der dazugehörenden Kurve. Dadurch kann der Anwender den Zusammenhang zwischen Tabelle, Graph und Auswahl der Kurven leicht erkennen. Nach dem Kreis folgt im selben Feld der Name des Reglers. 
	\item \empth{Kr}: Enthält den berechneten Wert für die Reglerverstärkung
	\item \empth{Tn}: Enthält den berechneten Tn-Reglerparameter 
	\item \empth{Tv}: Enthält den berechneten Tv-Reglerparameter
	\item \empth{Tp}: Enthält den berechneten Tp-Reglerparameter
	\item \empth{Überschwingen}: Enthält das gemessene Überschwingen in Prozent
\end{enumerate}

Für die nachträgliche Optimierung folgt unter der Tabelle ein Slider (\empth{Trimm für Zellwegermethode}), mit dem der Phasenwinkel manuell verändert werden kann (7). 
Die Tabelle des \empth{OutputPanels} ist eine JTable mit einem TableHeader und der Slider ein JSlider. 
\\
...
\textbf{GraphPanel:} Das \empth{GraphPanel} stellt die Schrittantworten der im \empth{GraphDisplayPanel} ausgewählten Regler-Typen dar. Die Achsen werden dabei automatisch skaliert und falls im \empth{GraphDisplayPanel} Anpassungen vorgenommen wurden, werden die einzelnen Kurven ein- oder ausgeblendet. Für das Anzeigen des Graphen wird ein XYPlot der Klasse \empth{JFreeChart} verwendet, der im BorderLayout platziert wurde. \\
...
\textbf{GraphDisplayPanel und GraphSettingPanel:} Im \empth{GraphDisplayPanel} können mit Hilfe der Checkboxen die Regler ausgewählt werden, welche im \empth{GraphPanel} angezeigt werden sollen (1). Rechts daneben wurde das \empth{GraphSettingPanel} platziert. Dieses zusätzliche Panel verfügt über drei Buttons mit den folgenden Funktionen:
\begin{itemize}
...
Ausserdem befindet sich am oberen Rand des Fensters die \empth{MenuBar} mit den beiden Schaltflächen \empth{Datei} und \empth{Hilfe}. Sie wurde direkt ins Frame gesetzt und wurde nicht in der Klasse \empth{View} hinzugefügt.\\

Das Menü \empth{Datei} besteht aus den folgenden Menüeinträgen:
\begin{itemize}
	\item \empth{Zur Mini-Version wechseln}: Wechselt von der Gesamtansicht zur Mini-Version. Elemente wie das \empth{GraphPanel}, das \empth{GraphDisplayPanel} und der Trimmer werden dabei ausgeblendet. Der Name dieses Menüeintrages ändert sich von Mini-Version zu Normal-Version.
	\item \empth{Export als PDF}: Öffnet ein Fenster, mit dem der Pfad und der Dokumentname ausgewählt werden kann. Nach der Bestätigung mit dem Button "`Speichern"' wird ein PDF generiert, welches die gesamte Ansicht des Tools exportiert.
	\item \empth{Schliessen}: Beendet die gesamte Applikation.
\end{itemize}

Das Menü \empth{Hilfe} besteht aus den folgenden Menüeinträgen:
\begin{itemize}
	\item \empth{Info}: Es öffnet sich ein Fenster mit Informationen zum Projekt und den Autoren.
	\item \empth{Hilfreiche Links}: Mit ihnen können Links und PDF-Dokumente geöffnet werden, welche weiterführende Information zur Reglerdimensionierung und der Zellweger-Methode bieten.
\end{itemize}
...
Im folgenden Ausschnitt aus dem Klassendiagramm ist der Zusammenhang der Klasse \empth{View} zu den Panels und den ganzen Swing-Komponenten dargestellt. Für das gesamte Klassendiagramm wird auf den Anhang \ref{klassendiagramm} verwiesen.
...
Wenn die Fenstergrösse durch den Anwender verändert wird, hat dies einen unterschiedlichen Einfluss auf die einzelnen Panels. Das \empth{OutputPanel} wird nur in der Vertikalen und das \empth{GraphDisplayPanel} und die \empth{MenuBar} nur in der Horizontalen verändert. Weiter wird das \empth{GraphPanel} in beide Richtungen gestreckt oder gestaucht. Das \empth{InputPanel} und das \empth{GraphSettingPanel} bleiben dabei unverändert. Zur Veranschaulichung wurde dieses Verhalten in der folgenden Grafik dargestellt. Die roten Pfeile zeigen dabei, in welche Richtungen sich die Panels verändern können.
...
\textbf{Mini-Version}\\
Durch eine Schaltfläche in der \empth{MenuBar} kann zur Mini-Version gewechselt werden. Diese reduzierte Ansicht verzichtet auf den Graphen und die dazugehörigen Panels und wurde speziell für fortgeschrittene Anwender implementiert. Die folgende Grafik zeigt die Mini-Version des Tools.
